	...
	public class MeasureRepository {
		...
		private static void CreateDatabase (SqliteConnection connection)
		{
			...
			using (var cmd = connection.CreateCommand ()) {
				...
				cmd.Parameters.AddWithValue ("@DataType", DataTypes.Continuous.DataType);
		...
		public static IEnumerable<Measure> GetMeasures (MeasureTypes type)
		{
			...
			using (var conn = GetConnection ()) {
				...
				using (var cmd = conn.CreateCommand ()) {
					cmd.CommandText = sql;
					cmd.Parameters.AddWithValue ("@MeasureType", (int)type);
		...
		private static Measure MeasureFromReader(SqliteDataReader reader) {
			MeasureTypes type = (MeasureTypes)reader.GetInt32(3);
			...
			using (var conn = GetConnection ()) {
				...
				using (var cmd = conn.CreateCommand ()) {
					...
					if (measure.Id < 0) {
						...
						cmd.Parameters.AddWithValue ("@MeasureType", (int)measure.MeasureType);
						cmd.Parameters.AddWithValue ("@Created", DateTime.Now);
